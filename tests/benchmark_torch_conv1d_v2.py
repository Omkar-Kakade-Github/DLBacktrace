...
if __name__ == "__main__":
    print("Starting DL_Backtrace Conv1D Layer Benchmark")
    print("=".ljust(80, "="))
    ...
    # Define a set of convolution sizes to test
    conv_configs = [
        {
            "name": "Small", 
            "kernel_size": 3, 
    ...
    # Run monotonic tests (ReLU-like) first
    print("\n" + (" Testing Monotonic Activation (ReLU-like) ").center(80, "-"))
    
    mono_results = []
    
    for config in conv_configs:
        desc = f"Monotonic ReLU-like - {config['name']} Conv1D"
        
        # For monotonic, we don't need activation functions
        act_np = None
        act_torch = None
        ...
        # Adapted for monotonic activation (passing dictionaries directly)
        mono_times = {}
        
        try:
            num_timer_repeats = 5
            num_runs = 10 if config['name'] != "Large" else 5  # Fewer runs for large config
            
            np_patch = np.random.rand(config['patch_size'], config['in_channels']).astype(np.float32)
            np_relevance = np.random.rand(config['out_channels']).astype(np.float32)
            np_weights = np.random.randn(config['kernel_size'], config['in_channels'], config['out_channels']).astype(np.float32)
            np_bias = np.random.randn(config['out_channels']).astype(np.float32)
    ...
    print("\n" + (" Testing Non-Monotonic Activations ").center(80, "-"))
    
    # Use a medium-sized configuration for activation tests
    medium_config = conv_configs[1]
    
    nonmono_results = []
    
    for act_conf in activation_configurations:
        desc = f"{act_conf['name']} - {medium_config['name']} Conv1D"
        times = run_benchmark_conv1d(
            desc=desc,
            kernel_size=medium_config["kernel_size"],
            in_channels=medium_config["in_channels"],
            out_channels=medium_config["out_channels"],
            patch_size=medium_config["patch_size"],
    ...
    all_results = mono_results + nonmono_results
    ...
    print("      PyTorch CUDA implementation should show the most benefit with larger input sizes.") 
